...
const ICONOS_GRUPOS: { label: string; iconos: string[] }[] = [
  { label: 'Hogar', iconos: ['🏠', '🏡', '🏢', '🏗️', '🛋️', '🛏️', '🚪', '🪟', '🪴', '🔑', '🔒', '🧹', '🪣', '🛠️', '🪚', '🔧'] },
  { label: 'Comida', iconos: ['🛒', '🥦', '🥕', '🍎', '🥩', '🐟', '🥚', '🧃', '☕', '🍺', '🍷', '🥐', '🍕', '🍔', '🍜', '🥗'] },
  { label: 'Restaurantes', iconos: ['🍽️', '🥡', '🍱', '🌮', '🍣', '🍦', '🧁', '🍰', '🫕', '🥘', '🍳', '🫙'] },
  { label: 'Transporte', iconos: ['🚗', '🚕', '🚌', '🚆', '🚇', '✈️', '🛵', '🚲', '🛺', '⛽', '🅿️', '🗺️'] },
  { label: 'Servicios', iconos: ['💡', '💧', '🔥', '🌐', '📡', '📺', '📱', '💻', '🖨️', '🔌', '📶', '🗑️'] },
  { label: 'Salud', iconos: ['💊', '🏥', '🩺', '💉', '🩹', '🧴', '🦷', '👓', '🩻', '🧠', '🫀', '🌡️'] },
  { label: 'Ocio', iconos: ['🎬', '🎮', '🎵', '🎭', '🎨', '📚', '🏋️', '⚽', '🎾', '🏊', '🎲', '🎉', '🎸', '🎯', '🏄', '🧗'] },
  { label: 'Ropa', iconos: ['👕', '👗', '👔', '👟', '👠', '👜', '🧢', '🧥', '👒', '💍', '⌚', '🕶️'] },
  { label: 'Mascotas', iconos: ['🐾', '🐕', '🐈', '🦜', '🐠', '🐇', '🦴', '🐾', '🏡', '🧸'] },
  { label: 'Educación', iconos: ['🎓', '📚', '🖊️', '🏫', '📐', '🔬', '🎒', '📝', '🖥️', '📖'] },
  { label: 'Finanzas', iconos: ['💰', '💳', '🏦', '📊', '📈', '💸', '🪙', '🏧', '🧾', '📉'] },
  { label: 'Varios', iconos: ['📦', '🎁', '🌟', '❤️', '💼', '🧳', '🔮', '⭐', '🪐', '🌈', '☀️', '🌙'] },
]
...
export function EditarGastoForm({
  ...
  const [form, setForm] = useState({
    ...
    tipo_division: gasto.tipo_division || '50/50',
  ...
  const montoCuota = montoTotal > 0 ? montoTotal / gasto.cuotas : 0
    ...
    <form onSubmit={handleSubmit} className="pb-28">
      ...
      <div className="space-y-4">
        ...
        >
              ...
              {metodoPagoOptions.map(opt => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (opt.value !== 'credito') set('tarjeta_id', '')
                  }}
                  className={cn(
          ...
          {/* Fecha de inicio (non-credit) */}
          {!isCredito && (
            <div className="space-y-1">
              <p className="text-[12px] text-muted-foreground">Fecha de inicio</p>
              <input
                type="date"
        ...
        {isCredito && (
          ...
          >
            ...
            {/* Read-only cuotas info */}
            <div className="flex items-center justify-between rounded-[10px] bg-muted/50 px-3 py-2.5">
              <span className="text-[13px] text-muted-foreground">Cantidad de cuotas</span>
              <span className="text-[13px] font-semibold text-foreground">
                {gasto.cuotas} {gasto.cuotas === 1 ? 'pago' : 'cuotas'}
              </span>
            </div>

            {gasto.cuotas > 1 && montoTotal > 0 && (
              <div className="flex justify-center">
                <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-muted">
                  <span className="text-[12px] text-muted-foreground font-medium">
                    {gasto.cuotas} cuotas de{' '}
